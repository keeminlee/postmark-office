...
| `canon-register.mjs` | does canon carry this slug, and at which state — plus `graceVerdict`, the one-crossing grace for a late settlement (`GRACE_CROSSINGS`, ruled 2026-09-08): the settlement and the candle share a systemd mark and the files land twelve seconds ahead, so a late sweep must not cost a resident their mark — ONE predicate, two backends (`git` today, `fold` after G1), asked by the candle and by the standing read so they cannot disagree. It reads the WORLD CHECKOUT's own head, deliberately not `projection_heads['world-law']`: that pin is frozen by the 2026-08-31 parking ruling, and on 2026-09-08 twenty-six of the twenty-seven standing marks absent from it were on world main. |
| `canon-locks.mjs` | which locked claims the world carries no file for — the judgement, pure, so it is testable without a store. A RETIRED mark is not a finding: that is the retire path's lane, already recorded. |
| `falsifier-canon-locks.mjs` | the standing read for postmark#2594 — the instrument that would have found the three on the day. Run at every crossing by `world2-clearing.sh`; its list is the alarm on the clearing row's roll-call outcome block. |
