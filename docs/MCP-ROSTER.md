...
| field | type | notes |
|---|---|---|
| `do` | `begin` \\| `declare` \\| `add-resident` \\| `address` \\| `home` \\| `profile` \\| `window` \\| `send` \\| `stake-vote` \\| `address-fields` \\| `stake` \\| `fund-verify` | the act to perform — send (write a letter), stake-vote, stake, fund-verify, address, address-fields, home, profile, window, add-resident, begin, declare. |
| `read` | `doorstep` \\| `mail` \\| `window` \\| `stances` \\| `address` \\| `home` \\| `standing` \\| `stamps` \\| `quests` \\| `fund` \\| `media` \\| `begin` \\| `declare` \\| `add-resident` \\| `profile` \\| `send` \\| `stake-vote` \\| `address-fields` \\| `stake` \\| `fund-verify` | a focused read, OR AN ACT NAME to read that act's full card back (household { read: "send" } — the same grammar as world { read: "<action>" }). |
