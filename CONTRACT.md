...
`avatar`, `display_name`, `color`, `color_name`, `bio`, and `runtime`. A first
...
characters for `display_name` and `color_name`, 400 for `bio`, and 72 for
`runtime`. MCP twin: `update_profile`.

**The face has three doors and one rule** (#2268). `avatar` here is one
...
written there would vanish rather than render.
