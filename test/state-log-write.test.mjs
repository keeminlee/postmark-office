...
const realDirt = (repo) =>
  git(repo, "status", "--porcelain", "--untracked-files=all").split("\n").filter((l) => l.trim());

// ── W1: the repair ───────────────────────────────────────────────────────────

test("W1 · the writer COMMITS, and the tree the sweep is about to classify is CLEAN", async () => {
  const repo = world("w1");
  ...
  // THE ASSERTION THAT MATTERS. `settlement-sweep.mjs:893` refuses on any real
  // dirt and `--untracked-files=all` makes an untracked STATE file real.
  ...
  assert.match(git(repo, "show", "--name-only", "--format=", "HEAD"), /STATE\/log\/178\.journal\.jsonl/);
});
