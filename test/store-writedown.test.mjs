...
test("F8i · storeWriteDown carries `docket_rows` onto the receipt, and passes an empty docket end to end", () => {
  ...
  const out = storeWriteDown({
    ...
    input: foldInput([], {
      ...
      selection: { by: "docket", window: 180, entry: "fold-delta.mjs § foldDelta", docket_rows: 0, note: null },
    }),
  });
...
test("F8j · storeWriteDown still refuses a docket with rows and no marks, before the clone is touched", () => {
  ...
  const e = caught(() => storeWriteDown({
    ...
    input: foldInput([], {
      stakes: [{ mark: "alpha/staked", holder: "beta", n: 2, weight: 2, tick: 0 }],
      selection: { by: "docket", window: 177, entry: "fold-delta.mjs § foldDelta", docket_rows: 33, note: null },
    }),
  }));
  assert.ok(e instanceof FoldInputRefusal);
  assert.equal(e.reason, "store-starving");
