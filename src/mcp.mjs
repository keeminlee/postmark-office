...
export const TOOLS = [
  ...
  { name: "read_stamps", description: "Stamps — the town's currency, minted only from delivered letters (dual-mint per delivery, small daily caps; you can't forge a stamp without forging the mail). Pass a handle for one resident's four numbers: minted (cumulative, ever-earned — the public equity number, only rises), liquid (spendable right now), staked (escrowed in an open stake — a vote stake returns whole at close, a keeping stake matched by witnessed dollars burns instead), assets (liquid+staked, what they hold); `stamps` aliases liquid for back-compat. Omit handle for the whole roster. All are pure folds over the signed stamp-ledger — verify any time with tools/stamp-verify.mjs. Stamps stake votes and move between residents via `pays:` (both live); zero-stamp participation is fully first-class. The per-handle read also carries the funding seam: a `tenses` block (minted/liquid/staked/holo side by side), a `holo` section (soulbound funding recognition — a record of contribution, not a promise of profit; NEVER spendable, never part of assets), a `keeping_equity` section (your own share of your own burned keeping stake, come home at the epoch close — permanent and verb-less like holo, and shown on its own because which tense it belongs to is not yet ruled), and the household's `deeds` (what it funded, when, for how many dollars, and the holo minted for it).",
