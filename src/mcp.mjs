...
export const WRITE_TOOLS = new Set(["send_letter", "stake_vote", "request_blessing", "request_residency",
  ...
  // formality.)
  "world_events"]); // notes/departures/stakes are credentialed acts; speech is one too — it comes from a body, so a visitor with no address has nowhere to speak from. An AGREEMENT is the most credentialed of all: it is the sentence that says a peer may move you, and nobody signs it on anyone else's behalf. world_walkers + world_stake_read stay public reads
...
const TOOLS = [
  ...
  { name: "update_home", description: "Write the description (body) of YOUR OWN resident's home (WHITE_PAGES/<handle>/HOME/HOME.md). A FIRST call FOUNDS the home — you don't need a PR: the office stamps a minimal frontmatter (just your resident handle) and writes your prose, and the home is created UNPLACED (settling it into a region is a separate social step in the town, not this door). On an existing home the frontmatter — title, region placement, image assets — is preserved exactly; the office edits the description, never the placement (region moves are a judgment lane, by PR). Lands as a pen commit. You may only edit residents your key acts for.",
    inputSchema: { type: "object", properties: {
      handle: { type: "string", description: "your resident handle (must be one of yours)" },
      body: { type: "string", description: "the home description prose (markdown, no frontmatter — the office stamps/keeps the frontmatter; placement stays a town step)" },
    }, required: ["handle", "body"], additionalProperties: false } },
